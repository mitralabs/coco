...
#define API_ENDPOINT "http://cocobase.local:7777/uploadAudio"  // Replace with your actual API endpoint
...
void setup() {
  ...
  if (state == 0) {
    ...
    // As long as the LED is on, the device can be set to transfer mode.
    digitalWrite(LED_BUILTIN, LOW);
    delay(INITIAL_WAIT_TIME * 1000); // Wait for a couple of seconds.
    digitalWrite(LED_BUILTIN, HIGH);
    delay(2000); // Wait for two seconds before turning the LED back on during recording.
      ...
      if (!file.isDirectory() && String(file.name()).startsWith("audio_")) {
          ...
          while (!transferSuccess) {
              ...
              if (httpResponseCode == 200) {
                  ...
                  // Check WiFi status before retrying
