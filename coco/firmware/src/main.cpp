    ...
    if (WIFIconnected) {
      ...
      if (xSemaphoreTake(uploadMutex, 0) == pdTRUE) {
        ...
        if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(5000)) == pdTRUE) {
          File root = SD.open(RECORDINGS_DIR);
          ...
          if (root && root.isDirectory()) {
            ...
            // Look for a WAV file to upload
            while (file) {
              String filename = file.name();
              if (!file.isDirectory() && filename.endsWith(".wav")) {
                // Found a file to upload
                foundFile = true;
                log("Found audio file to upload: " + filename);
                
                // Save the full path for later deletion
                uploadBuffer.filename = String(RECORDINGS_DIR) + "/" + filename;
            ...
            if (foundFile && uploadBuffer.buffer) {
              ...
              log("No audio files to upload");
              wavFilesAvailable = false;  // Set the flag to false if no .wav files are found
            }
          } else {
            log("Failed to open recordings directory");
            xSemaphoreGive(sdMutex);
          }
        } else {
          log("Could not get SD card mutex for file upload check");
        }
