...
#define TRANSFER_COMPLETE_ENDPOINT "http://cocobase.local:3030/TransferComplete"
#define API_KEY "test" // Replace with actual API key
...
void loop() {
  switch (currentState) {
    ...
    case TRANSFERRING:
      ...
      while (file) {
        if (!file.isDirectory() && String(file.name()).startsWith("audio_")) {
          ...
          int session = fileName.substring(6, fileName.indexOf('_', 6)).toInt(); // Adjust indices if format changes

          if (session!= currentSessionTransfer) {
            if (currentSessionTransfer!= -1) { // If it's not the first session
                sessionCompletelyTransfered(currentSessionTransfer); // Call for previous session
            }
    ...
    case SLEEP:
      ...
      esp_bt_controller_disable();
      log("Sleeping");
      // setCpuFrequencyMhz(10); // Maybe setting CPU to lowest clock. 10 seems to restart the device, maybe too low...
...
void sessionCompletelyTransfered(int sessionId) {
    ...
    String endpoint = String(TRANSFER_COMPLETE_ENDPOINT) + "?recording_session=" + String(sessionId);
