...
def handle_terminate(sig, frame):
    ...
    logger.info(f"Received signal {sig}, initiating shutdown...", extra={"event_type": "shutdown_signal", "signal": sig})
    shutdown_event.set()
...
async def db_lifespan(server: FastMCP):
    ...
    if not database_url:
        ...
        logger.critical("DATABASE_URL environment variable is not set", extra={"event_type": "config_error", "variable": "DATABASE_URL"})
        sys.exit(1)
    ...
    if embedding_api not in ["ollama", "openai"]:
        ...
        logger.critical(f"Invalid COCO_EMBEDDING_API: {embedding_api}", extra={
    ...
    if embedding_api == "openai" and not os.getenv("OPENAI_API_KEY"):
        ...
        logger.critical("OPENAI_API_KEY environment variable is required when COCO_EMBEDDING_API=openai", extra={
            "event_type": "config_error",
    ...
    async def check_parent_process():
        ...
        while not shutdown_event.is_set():
            try:
                ...
                logger.info("Parent process is no longer running, initiating shutdown...", extra={"event_type": "parent_process_exit"})
                shutdown_event.set()
                break
    ...
    try:
        ...
        logger.info(f"Attempting to connect to database pool: {database_url.split('@')[-1]}", extra={"event_type": "db_pool_init_start"})
        pool = await asyncpg.create_pool(database_url)
        logger.info(f"Successfully connected to database pool: {database_url.split('@')[-1]}", extra={"event_type": "db_pool_init_success"})
        ...
        logger.info(f"Initializing Coco LanguageModelClient", extra={
        ...
        if not shutdown_event.is_set():
            try:
                ...
                logger.info("Server running, waiting for shutdown signal...", extra={"event_type": "waiting_for_shutdown"})
                await asyncio.wait_for(shutdown_event.wait(), timeout=5.0)
            except asyncio.TimeoutError:
                # logger.error("Timeout waiting for shutdown signal, proceeding with cleanup") # Old logging
                logger.warning("Timeout waiting for shutdown signal, proceeding with cleanup anyway", extra={"event_type": "shutdown_timeout"})
            
    except Exception as e:
        # logger.error(f"Error during startup: {e}") # Old logging
        logger.critical(f"Fatal error during server startup: {e}", exc_info=True, extra={
        ...
        logger.info("Shutting down MCP server, cleaning up resources...", extra={"event_type": "shutdown_start"})
        ...
        if pool is not None:
            ...
            logger.info("Closing database connection pool...", extra={"event_type": "db_pool_close_start"})
            await pool.close()
            logger.info("Database connection pool closed.", extra={"event_type": "db_pool_close_success"})
            
        # logger.error("Cleanup complete, server shutting down") # Old logging
        logger.info("Resource cleanup complete. Server shut down.", extra={"event_type": "shutdown_complete"})
...
async def get_table_schema(table_name: str) -> str:
    ...
    logger.info("Starting resource call", extra={
    ...
    try:
        ...
        logger.info("Attempting direct database connection", extra={
            "event_type": "db_direct_connect_start",
            "resource_uri": resource_uri,
            "database_url": DATABASE_URL.split('@')[-1] # Log target db without credentials
        })
        conn = await asyncpg.connect(DATABASE_URL)
        logger.info("Successfully established direct database connection", extra={
        ...
        """
        logger.info("Executing schema query", extra={
        ...
        if not results:
            ...
            logger.warning(f"Schema information not found for table '{table_name}'", extra={
        ...
        duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
        logger.info("Resource call successful", extra={
        ...
        duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
        logger.error("Resource call failed", exc_info=True, extra={
        ...
        if conn and not conn.is_closed():
            try:
                await conn.close()
                logger.info("Direct database connection closed after error", extra={
                    "event_type": "db_direct_close_on_error",
                    "resource_uri": resource_uri
                })
            except Exception as close_err:
                logger.error("Failed to close database connection after error", exc_info=True, extra={
        ...
        if conn and not conn.is_closed():
            await conn.close()
            logger.info("Direct database connection closed", extra={
                "event_type": "db_direct_close",
                "resource_uri": resource_uri
            })
...
async def execute_pgvector_query(
    ...
    logger.info("Starting tool execution", extra={
    ...
    if semantic_string and not placeholder_present:
        ...
        logger.warning("'semantic_string' provided, but query missing '$1' placeholder.", extra={
            "event_type": "tool_call_warning",
            "tool_name": tool_name,
            "warning_details": {"message": "Semantic string provided but not used in query."},
            "tool_input": tool_input
        })
    ...
    try:
        ...
        if semantic_string and placeholder_present:
            ...
            try:
                ...
                embedding_duration_ms = (asyncio.get_event_loop().time() - embedding_start_time) * 1000
                embedding_dimensions = len(embedding_vector) if embedding_vector is not None else 0
                
                logger.info(f"Embedding generated successfully", extra={
                ...
                if embedding_dimensions > 0:
                    ...
                    logger.info("Using embedding vector for pgvector", extra={
                ...
                duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
                embedding_duration_ms = (asyncio.get_event_loop().time() - embedding_start_time) * 1000
                logger.error("Embedding generation failed", exc_info=True, extra={
                    "event_type": "tool_call_error",
                    "tool_name": tool_name,
                    "error_stage": "embedding",
                    "error_details": {"type": type(embed_err).__name__, "message": str(embed_err)},
                    "tool_input": tool_input,
        ...
        logger.info("Executing database query", extra={
            ...
            "sql": sql_query[:500] + ("..." if len(sql_query) > 500 else "") # Log truncated query
        })
        ...
        async with pool.acquire() as conn:
            ...
            try:
                ...
                logger.info("Registered vector type codec for connection", extra={
                    "event_type": "db_codec_register_success",
                    "tool_name": tool_name
                })
            except Exception as codec_error:
                # Log as warning, as the query might still work
                logger.warning(f"Could not register vector type codec: {codec_error}", extra={
                    "event_type": "db_codec_register_warning",
                    "tool_name": tool_name,
                    "error_details": {"type": type(codec_error).__name__, "message": str(codec_error)}
                })
            ...
            async with conn.transaction(readonly=True, isolation='read_committed'):
                try:
                    ...
                    db_query_duration_ms = (asyncio.get_event_loop().time() - db_query_start_time) * 1000
                    ...
                    logger.error("SQL syntax error during query execution", exc_info=True, extra={
                        "event_type": "tool_call_error",
                        "tool_name": tool_name,
                    ...
                    duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
                    db_query_duration_ms = (asyncio.get_event_loop().time() - db_query_start_time) * 1000
                    logger.error("Error during database query execution", exc_info=True, extra={
                        "event_type": "tool_call_error",
                        "tool_name": tool_name,
        ...
        duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
        logger.info("Tool execution successful", extra={
        ...
        duration_ms = (asyncio.get_event_loop().time() - start_time) * 1000
        logger.error("Unhandled error during tool execution", exc_info=True, extra={
            "event_type": "tool_call_error",
            "tool_name": tool_name,
            "error_stage": "outer_exception",
            "error_details": {"type": type(outer_err).__name__, "message": str(outer_err)},
            "tool_input": tool_input,
            "duration_ms": duration_ms
        })
        return json.dumps({
            "error": f"An unexpected error occurred: {str(outer_err)}",
            "results": []
        }, indent=2)
