...
def get_vector_dim_from_db(engine, session, logger):
...
def check_vector_dimensions(engine, session, logger):
    """Check and handle vector dimension changes

    Returns:
        bool: True if table needs to be recreated, False otherwise
    """
    table_emb_dim = get_vector_dim_from_db(engine, session, logger)
